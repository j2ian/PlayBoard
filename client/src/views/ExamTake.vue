...
onMounted(async () => {
  try {
    ...
    if (route.query.playbook) {
      console.log('[ExamTake] 從 PlayBook 進入，playbook ID:', route.query.playbook)
      ...
      try {
        ...
        if (playbookSlug) {
          ...
          if (response.data.success) {
            ...
            console.log('[ExamTake] PlayBook theme:', theme)
          }
        }
      } catch (error) {
        console.error('[ExamTake] 取得 PlayBook 主題失敗，使用預設主題:', error)
      }
    } else {
      console.log('[ExamTake] 獨立進入，使用預設主題')
    }

    // 動態載入對應主題的元件
    const component = await loadThemeComponent(theme, 'ExamTake')
    console.log('[ExamTake] 元件載入成功，theme:', theme)
    ThemeComponent.value = markRaw(component)
  } catch (error) {
    console.error('[ExamTake] 載入失敗:', error)
  } finally {
    loading.value = false
  }
})
