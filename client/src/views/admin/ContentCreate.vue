<template>
  <AdminLayout title="新增內容">
    <el-card>
      <template #header>
        <div class="flex items-center justify-between">
          <span class="text-lg font-semibold">建立新內容</span>
      ...
      >
        ...
        <div class="mb-6">
          ...
          <el-form-item label="內容類型" prop="contentType" required>
            <el-select v-model="content.contentType" placeholder="請選擇內容類型">
              <el-option
        ...
        <!-- 提交按鈕 -->
        <el-form-item>
          <el-button type="primary" @click="submitContent" :loading="loading">建立內容</el-button>
...
const content = ref({
  ...
  excerpt: '',
  content: '',
  contentType: 'html', // 預設使用HTML以支援富文本編輯器
  category: '一般',
  tags: [],
  featuredImage: null,
  images: [],
...
// 標題改變時
const onTitleChange = () => {
  // 如果slug為空或由標題自動生成，則更新slug
  if (!content.value.slug || isSlugFromTitle()) {
    generateSlugFromTitle()
  }
}

// 檢查slug是否由標題生成
const isSlugFromTitle = () => {
  const generatedSlug = ContentService.generateSlug(content.value.title)
  return content.value.slug === generatedSlug
}
...
const submitContent = async () => {
  ...
  try {
    ...
    const response = await ContentService.createContent(content.value)
    
    if (response.data.success) {
      ElMessage.success('內容建立成功')
      router.push('/admin/contents')
    }
  } catch (error) {
    console.error('建立內容失敗:', error)
    ElMessage.error('建立內容失敗')
  } finally {
    loading.value = false
  }
}
