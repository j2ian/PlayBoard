<template>
  <AdminLayout title="編輯測驗">
    <el-card>
      <template #header>
        <div class="flex items-center justify-between">
          <span class="text-lg font-semibold">編輯測驗</span>
      ...
      <el-form :model="exam" ref="examForm" :rules="rules" label-width="120px" class="max-w-2xl mx-auto">
        ...
        <el-form-item>
          <el-button type="primary" @click="submitExam" :loading="loading">儲存變更</el-button>
...
const loadExam = async () => {
  ...
  try {
    ...
    // 轉換 questions 為 id 陣列
    exam.value = {
      ...data.data,
      questions: Array.isArray(data.data.questions)
        ? data.data.questions.map(q => (typeof q === 'string' ? q : q._id))
        : [],
      timeLimit: data.data.timeLimit || 0,
      passingScore: data.data.passingScore || 60,
      allowFailToContinue: data.data.allowFailToContinue || false,
    }
  } catch (error) {
...
const fetchQuestions = async (query = '') => {
  ...
  try {
    const response = await QuestionService.getAllQuestions({ search: query, pageSize: 50 })
...
const submitExam = async () => {
  ...
  await examForm.value.validate(async (valid) => {
    ...
    loading.value = true
    try {
      await ExamService.updateExam(route.params.id, exam.value)
      ElMessage.success('測驗已更新！')
      goBack()
    } catch (error) {
      ElMessage.error(error?.response?.data?.message || '更新失敗，請檢查欄位或稍後再試')
    } finally {
      loading.value = false
    }
  })
