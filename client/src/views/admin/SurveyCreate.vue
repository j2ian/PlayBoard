<template>
  <AdminLayout title="新增滿意度調查">
    <el-card>
      <template #header>
        <div class="flex items-center justify-between">
          <span class="text-lg font-semibold">建立新問卷</span>
      ...
      <el-form :model="survey" ref="surveyForm" :rules="rules" label-width="120px" class="max-w-4xl mx-auto">
        ...
        <!-- 提交按鈕 -->
        <el-form-item>
          <el-button type="primary" @click="submitSurvey" :loading="loading">建立問卷</el-button>
...
const submitSurvey = async () => {
  ...
  try {
    ...
    const response = await SurveyService.createSurvey(survey.value)
    
    if (response.data.success) {
      ElMessage.success('問卷建立成功')
      router.push('/admin/surveys')
    }
  } catch (error) {
    console.error('建立問卷失敗:', error)
    ElMessage.error('建立問卷失敗')
  } finally {
    loading.value = false
  }
}
