<!-- 第一頁 -->
<template>
  <div class="min-h-screen ocean-bg-menu">
    <!-- 載入中 -->
    ...
    <div v-else class="max-w-2xl mx-auto px-4 py-8 fixed bottom-0 left-0 right-0">
      ...
      <div class="bg-white **opacity-50** rounded-lg shadow-md p-6 mb-6">
        ...
        <div class="flex gap-3">
          ...
          <el-button v-else-if="!userProgress.isCompleted" type="primary" size="large" @click="continuePlayBook">
            <el-icon>
              <CaretRight />
            </el-icon>
            繼續學習
          </el-button>
          <el-button v-else type="success" size="large" @click="reviewPlayBook">
            <el-icon>
              <View />
            </el-icon>
            重新檢視
          ...
          <!-- 逐步模式切換按鈕 -->
          <el-button v-if="playbook.displayType !== 'stepByStep' && userProgress && !userProgress.isNewUser"
            size="large" type="info" plain @click="switchToStepMode">
            <el-icon>
              <Right />
            </el-icon>
            逐步模式
          ...
          <el-button v-if="userProgress && !userProgress.isNewUser" size="large" plain @click="resetProgress">
            <el-icon>
              <RefreshRight />
            </el-icon>
            重新開始
      ...
      <div v-if="playbook && playbook.displayType === 'overview'" class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="space-y-4">
          <div v-for="(step, index) in (playbook?.steps || [])" :key="step.stepNumber" class="relative">
            <!-- 步驟連接線 -->
            <div v-if="index < (playbook?.steps?.length || 0) - 1"
              class="absolute left-6 top-12 w-0.5 h-16 bg-gray-200 z-0"></div>

            <!-- 步驟卡片 -->
            <div class="relative z-10 flex items-start gap-4 p-4 rounded-lg border-2 transition-all"
              :class="getStepCardClass(step)">
              <!-- 步驟編號 -->
              <div class="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center font-semibold"
              ...
              <div class="flex-1">
                ...
                <div class="flex gap-2">
                  <!-- 開始按鈕 -->
                  <el-button v-if="canAccessStep(step.stepNumber) && !isStepCompleted(step.stepNumber)" type="primary"
                    size="small" @click="goToStep(step)">
                    <el-icon>
                      <CaretRight />
                    </el-icon>
                  ...
                  <!-- 查看按鈕（已完成的步驟） -->
                  <el-button v-else-if="isStepCompleted(step.stepNumber)" type="success" size="small" plain
                    @click="goToStep(step)">
                    <el-icon>
                      <View />
                    </el-icon>
      ...
      <!-- 完成慶祝 -->
      <div v-if="userProgress && userProgress.isCompleted"
        class="bg-green-50 border border-green-200 rounded-lg p-6 mt-6">
