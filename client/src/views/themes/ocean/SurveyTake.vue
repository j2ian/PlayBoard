<template>
  <div class="min-h-screen py-8 ocean-bg-none flex items-center justify-center">
    <div class="max-w-4xl w-full max-h-full p-8 overflow-y-auto ocean-survey-container ocean-scrollbar">
      <!-- 載入中 -->
      ...
      <el-card v-else-if="!survey" class="text-center">
        <div class="py-8">
          ...
          <p class="text-gray-600">你要填寫的問卷可能已被刪除或不存在。</p>
        </div>
      </el-card>
      ...
      <el-card v-else-if="submitted" class="text-center">
        <div class="py-8">
          ...
          <p class="text-gray-600">感謝你的參與！你的回答已成功提交。</p>
      ...
      <div v-else>
        ...
        <el-card class="mb-6">
          ...
          <el-form :model="studentInfo" :rules="studentInfoRules" ref="studentInfoForm">
            <el-form-item label="你的姓名" prop="name" required>
              <el-input v-model="studentInfo.name" placeholder="請輸入你的姓名" :disabled="submitting || isPlayBookMode" />
            </el-form-item>
          </el-form>
        </el-card>
          ...
          <el-card v-for="(question, index) in survey.questions" :key="index" class="mb-6">
            ...
            <div v-if="question.type === 'likert'">
              <div class="mb-4">
                <div class="grid grid-cols-5 gap-2">
                  <div v-for="option in likertOptions" :key="option.value"
                    class="text-center p-2 border rounded cursor-pointer transition-all" :class="{
              ...
              <div v-if="responses[index]" class="text-sm text-gray-600 text-center">
                你選擇了：{{ getLikertLabel(responses[index]) }}
              </div>
            </div>

            <!-- 文字題 -->
            <div v-else-if="question.type === 'text'">
              <el-input v-model="responses[index]" type="textarea" :rows="4" placeholder="請輸入你的回答..."
                :disabled="submitting" />
            </div>
            ...
            <div v-else-if="question.type === 'multiple'">
              <el-checkbox-group v-model="responses[index]" :disabled="submitting">
                <div class="space-y-2">
                  <el-checkbox v-for="(option, optionIndex) in question.options" :key="optionIndex" :value="optionIndex"
                    class="w-full">
                    <span class="ml-2">{{ option }}</span>
                  </el-checkbox>
                </div>
        ...
        <el-card class="text-center">
          <el-button type="primary" size="large" @click="submitSurvey" :loading="submitting" :disabled="!canSubmit">
            {{ submitting ? '提交中...' : '提交問卷' }}
          </el-button>
...
const studentInfoRules = {
  name: [{ required: true, message: '請輸入你的姓名', trigger: 'blur' }]
}
...
const submitSurvey = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      ElMessage.success('問卷提交成功，感謝你的參與！')
