...
| 欄位名稱 | 類型 | 必填 | 預設值 | 說明 |
|---------|------|------|--------|------|
| `playBook` | ObjectId | 是 | - | PlayBook 的 ID，關聯到 PlayBook 模型 |
| `userId` | String | 是 | - | 用戶唯一識別符 |
| `userName` | String | 否 | '匿名用戶' | 用戶顯示名稱 |
| `currentStep` | Number | 否 | 1 | 當前正在進行的步驟編號 |
| `completedSteps` | [Number] | 否 | [] | 已完成的步驟編號陣列 |
| `stepResults` | Map | 否 | new Map() | 每個步驟的詳細結果記錄 |
| `totalSteps` | Number | 是 | - | PlayBook 的總步驟數 |
| `startedAt` | Date | 否 | Date.now | 開始學習的時間 |
| `lastActiveAt` | Date | 否 | Date.now | 最後活動時間 |
| `completedAt` | Date | 否 | - | 完成時間（僅在完成時設置） |
| `timeSpent` | Number | 否 | 0 | 總花費時間（秒） |
| `isCompleted` | Boolean | 否 | false | 是否已完成整個 PlayBook |
| `completionRate` | Number | 否 | 0 | 完成率百分比 |
