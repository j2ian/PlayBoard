...
- `setup.js`: 這個檔案會在所有測試執行前，自動設定好與記憶體資料庫的連線，並在所有測試結束後清除資料與連線。你不需要在測試檔案中手動引入它，Jest 會自動處理。
...
- **執行 (Act)**: 執行你想要測試的程式碼 (例如：儲存一個 Model 實例)。
...
測試失敗或錯誤情境同樣重要。你可以透過 `try...catch` 區塊來捕捉預期中的錯誤，並驗證錯誤的類型和訊息。
...
Jest 將會自動尋找所有 `*.test.js` 檔案並執行它們。你將會在終端機中看到詳細的測試報告，包含哪些測試通過、哪些失敗。

---

遵循本指南，你將能夠為專案建立穩定且可靠的測試，確保程式碼的品質。
