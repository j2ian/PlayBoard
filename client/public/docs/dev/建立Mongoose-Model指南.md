...
本文件將引導你完成在 `server/models` 目錄下建立新 Mongoose Model 的完整步驟。請遵循以下指南以確保程式碼的一致性與可維護性。
...
Mongoose Schema 支援中間件，讓你可以在特定操作 (如 `save`, `remove`) 前後執行自訂邏輯。最常見的用途是在儲存前加密密碼或更新 `updatedAt` 時間戳。
...
你可以為 Schema 增加自訂的實例方法，這些方法將在所有從此 Model 建立的文件實例上可用。
